...
// const provider = new Web3.providers.HttpProvider('https://ropsten-rpc.linkpool.io/');
    ...
    try {
        switch (m.type) {
            case 'CREATE_ACCOUNT': {
                ...
                let response = {CMD: 'RES_CREATE_ACCOUNT', data};
            ...
            case 'IMPORT_PRIVATE_KEY_REQ': {
                let data = privateToAccount(m.payload.privateKey);
                let response = {    CMD: 'IMPORT_PRIVATE_KEY_RES', data   };
                web3Event.emit('res-web3-event', response);
                break;
            }

            case 'IMPORT_JSON_KEYSTORE_REQ': {
                let activated = keyStoreDecrypt(m.payload.json_keystore, m.payload.password_import);
                let response = { CMD: 'IMPORT_JSON_KEYSTORE_RES', data: activated };
                web3Event.emit('res-web3-event', response);
                break;
            }
            ...
            case 'IMPORT_NEW_JSON_KEYSTORE_REQ': {
                let activated = keyStoreDecrypt(m.payload.json_keystore, m.payload.password_import);
                let response = {CMD: 'IMPORT_NEW_JSON_KEYSTORE_RES', data: activated};
                web3Event.emit('res-web3-event', response);
                break;
            }
            ...
            case 'CHECK_BALANCE_OF': {
                ...
                const makeBalanceOfEthPromise = new Promise(
                    function (resolve, reject) {
                        ...
                        contract.methods.balanceOf(m.payload).call()
                            .then(res => {
                                let token = web3.utils.fromWei(res);
                ...
                makeBalanceOfEthPromise.then(data => {
                    ...
                    let response = {CMD: 'RES_CHECK_BALANCE_OF', data};
                    web3Event.emit('res-web3-event', response);
                }).catch(function (error) {
                    console.log(error);
                    let response = { CMD: 'RES_CHECK_BALANCE_OF', data: error };
                    web3Event.emit('res-web3-event', response);
                });
                break;
            }
            ...
            case 'SEND_ETH_TRANSACTION': {
                ...
                SendETH(m.payload).then(function (data) {
                    ...
                    let response = {CMD: 'RES_SEND_ETH_TRANSACTION', data};
                    web3Event.emit('res-web3-event', response);
                }).catch(function (error) {
                    console.log('Fail Transaction Promise', error);
                    let response = {CMD: 'RES_SEND_ETH_TRANSACTION', data: 'FAIL WHILE TRANSACTION'};
                    web3Event.emit('res-web3-event', response);
                });
                break;
            }
            ...
            case 'SEND_CON_TRANSACTION': {
                ...
                SendCON(m.payload).then(function (data) {
                    console.log('Get Promise', data);
                    let response = {CMD: 'RES_SEND_CON_TRANSACTION', data};
                    web3Event.emit('res-web3-event', response);
                }).catch(function (error) {
                    console.log('Fail Transaction Promise', error)
                });
            ...
            case 'GET_TX_RECEIPT': {
                ...
                GetTxReceipt(m.payload).then(function (data) {
                    console.log('Get Promise', data);
                    let response = {CMD: 'RES_TX_RECEIPT', data};
                    web3Event.emit('res-web3-event', response);
                }).catch(function (error) {
                    console.log('Fail Transaction Receipt', error);
                    let response = {CMD: 'RES_TX_RECEIPT', error};
                    web3Event.emit('res-web3-event', response);
                });
                break;
            }
